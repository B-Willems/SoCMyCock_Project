...
static u16 _TriangleTest[AUDIOPLAYER_FIFO_BUFFER_SIZE] = {
...
static u16 _SpecialTest[AUDIOPLAYER_FIFO_BUFFER_SIZE] = {
 ...
 */
static void AudioPlayer_ChangeFreq(const u32 SampleRate,char octave)
{
//	if(SampleRate == 44100)
//	{
...
//		_ClkWiz.CLKFBOUT_MULT = 23;
//		_ClkWiz.CLKFBOUT_FRAC_Multiply = 0;
//		_AudioClock.DIVIDE = 2;
//		_AudioClock.FRAC_Divide = 0;
//	}

		xil_printf("	Use clock setting 3...\n\r");
		_ClkWiz.DIVCLK_DIVIDE = 4 - octave;
		_ClkWiz.CLKFBOUT_MULT = 23;
		_ClkWiz.CLKFBOUT_FRAC_Multiply = 0;
		_AudioClock.DIVIDE = SampleRate;
		_AudioClock.FRAC_Divide = 0;
...
void AudioPlayer_SendTestSignal(u32 Cycles,short int type, char octave )
{
	...
	if(type ==1){ AudioPlayer_ChangeFreq(2,octave);if(octave >3){AudioPlayer_ChangeFreq(1,octave-3); } 	AudioPlayer_CopyBuffer(_TestSignal);}
	if(type ==2){ AudioPlayer_ChangeFreq(2,octave);if(octave >3){AudioPlayer_ChangeFreq(1,octave-3); }	AudioPlayer_CopyBuffer(_SquareTest);}
	if(type ==3){ AudioPlayer_ChangeFreq(2,octave);if(octave >3){AudioPlayer_ChangeFreq(1,octave-3); }	AudioPlayer_CopyBuffer(_TriangleTest);}
	if(type ==4){ AudioPlayer_ChangeFreq(2,octave);if(octave >3){AudioPlayer_ChangeFreq(1,octave-3); }	AudioPlayer_CopyBuffer(_SpecialTest);}
	//AudioPlayer_CopyBuffer();
}
